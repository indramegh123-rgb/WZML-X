...
from ..mirror_leech_utils.upload_utils.multi_upload import MultiUphosterUpload
...
class TaskListener(TaskConfig):
    ...
    async def clean(self):
        with suppress(Exception):
            ...
            await gather(
                TorrentManager.aria2.purgeDownloadResult(),
                delete_status(),
            )
...
""",
 ...
 """,
    ...
    async def _on_download_complete(self):
            ...
            async with same_directory_lock:
                while True:
                    async with task_dict_lock:
                        if (
                            self.mid
                            not in self.same_dir[self.folder_name]["tasks"]
                        ):
                            return
                        ...
                        if (
                            ...
                            if self.same_dir[self.folder_name]["total"] > 1:
                                ...
                                LOGGER.info(
                                    f"Moving files from {self.mid} "
                                    f"to {des_id}"
                                )

                                await move_and_merge(
                                    spath,
                                    des_path,
                                    self.mid,
                                )
        ...
        if self.folder_name:
            self.name = (
                self.folder_name.strip("/")
                .split("/", 1)[0]
            )

        if not await aiopath.exists(
            f"{self.dir}/{self.name}"
        ):
            try:
        ...
        if self.seed:
            ...
            await create_recursive_symlink(
                self.dir,
                self.up_dir,
            )

            LOGGER.info(
                f"Shortcut created: {dl_path} -> {up_path}"
            )
        ...
        if (
            (
                hasattr(self, "metadata_dict")
                and self.metadata_dict
            )
            or (
                hasattr(self, "audio_metadata_dict")
                and self.audio_metadata_dict
            )
            or (
                hasattr(self, "video_metadata_dict")
                and self.video_metadata_dict
            )
        ):
        ...
        if self.screen_shots:
            up_path = await self.generate_screenshots(
                up_path
            )
        ...
        if add_to_queue:
            LOGGER.info(
                f"Added to Queue/Upload: {self.name}"
            )
            ...
            LOGGER.info(
                f"Start from Queued/Upload: {self.name}"
            )
        ...
        if self.is_yt:
            ...
            LOGGER.info(
                f"Uphoster Upload Name: {self.name}"
            )

            uphoster_service = self.user_dict.get(
                "UPHOSTER_SERVICE",
                "gofile",
            )
            ...
            async with task_dict_lock:
                task_dict[self.mid] = UphosterStatus(
                    self,
                    ddl,
                    gid,
                    "up",
            ...
            LOGGER.info(
                f"Gdrive Upload Name: {self.name}"
            )
            ...
            LOGGER.info(
                f"Mega Upload Name: {self.name}"
            )

            mega_email = (
                self.user_dict.get("MEGA_EMAIL")
                or ""
            )

            mega_password = (
                self.user_dict.get("MEGA_PASSWORD")
                or ""
            )

            await add_mega_upload(
                self,
                up_path,
                mega_email,
                mega_password,
            ...
            LOGGER.info(
                f"Rclone Upload Name: {self.name}"
            )
    ...
    async def on_upload_complete(
        ...
        if (
            self.is_super_chat
            and (
                Config.INC_TASK_NOTIFY
                or Config.INC_TASK_RESUME
            )
            and Config.DATABASE_URL
        ):
        ...
        msg = (
            ...
            f"\n┟ <b>Task Size</b> → "
            f"{get_readable_file_size(self.size)}"
            f"\n┠ <b>Time Taken</b> → "
            f"{get_readable_time(time() - self.message.date.timestamp())}"
            f"\n┠ <b>In Mode</b> → {self.mode[0]}"
            f"\n┠ <b>Out Mode</b> → {self.mode[1]}"
        )

        LOGGER.info(
            f"Task Done: {self.name}"
        )
        ...
        if self.is_yt:
            ...
            if mime_type == "Folder/Playlist":
                ...
                user_message = (
                    f"{self.tag}\n"
                    f"Your playlist ({files} videos) "
                    "has been uploaded to YouTube successfully!"
                )
            ...
            # ---------------------------------------------------------
            # LE​​ECH COMPLETE MESSAGE
            # ---------------------------------------------------------

            task_size = get_readable_file_size(
                self.size
            )
            ...
            # This uses the actual Telegram User ID.
            user_mention = (
            ...
            if self.bot_pm:
                pmsg = (
                    ...
                    "┃\n"
                    "┃ 📬 Your file(s) have been\n"
                    "┃    sent to your Private Message.\n"
                    "┃\n"
                    ...
                    if Config.MEDIA_STORE and (
                        ...
                        if len(parts) == 2:
                            ...
                            flink = (
                                ...
                                f"{TgClient.BNAME}"
                                "?start="
                                f"{encode_slink('file' + chat_id + '&&' + msg_id)}"
                            )
                            ...
                            fmsg += (
                                ...
                                "</a>\n"
                                "┃ └─ 🔗 "
                                f"<a href='https://t.me/share/url?url={flink}'>"
                                "𝐒𝐡𝐚𝐫𝐞 𝐋𝐢𝐧𝐤"
                                "</a>\n"
                            )
            ...
            if (
                ...
                for service, result in link.items():
                    if "error" in result:
                        ...
                        multi_links.append(
                            (
                                f"{service.capitalize()} Link",
                                result["link"],
                            )
                        )

                multi_link_msg = (
                    multi_link_msg.strip()
                )
            ...
            if (
                ...
                if (
                    ...
                    url_path = rutils.quote(
                        f"{rpath}"
                    )

                    share_url = (
                        f"{Config.RCLONE_SERVE_URL}/"
                        f"{remote}/{url_path}"
                    )
                ...
                if not rclone_path and dir_id:
                    ...
                    if INDEX_URL and self.name:
                        safe_name = rutils.quote(
                            self.name.strip("/")
                        )

                        share_url = (
                            f"{INDEX_URL}/{safe_name}"
                        )
            ...
            if (
                hasattr(Config, "MIRROR_LOG_ID")
                and Config.MIRROR_LOG_ID
            ):
                await send_message(
                    Config.MIRROR_LOG_ID,
                    msg,
                    button,
                )
